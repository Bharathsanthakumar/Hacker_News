	...
	<body>
		...
		<table style="margin-left: auto; margin-right: auto; border-spacing:0px; width:90%" >
			...
			<tr style="background-color:beige;">
				<td	colspan="10" style="text-align:center;font-size:30px;">POST</td>
			</tr>
			<tr style="background-color:beige;">
				<td colspan="8" style="text-align:center;font-size:26px; font-weight: bold;"><br><%=item.getTitle()%><br><br></td>
			</tr>
			<tr style="background-color:beige;">
				<td colspan="8" style="text-align:justify;"><%=item.getContent()%><br><br></td>
			</tr>
			...
			<tbody style="background-color:silver; vertical-align: text-top;" >
				...
				<tr>
					<td>
						...
						<button type="button"  onclick="location.href = './ItemServlet?parentid=<%=itemId%>&commentid=<%=item.getId()%>&action=delete&src=AskItem.jsp'">DELETE</button>
						
						<form  class="form" action="./ItemServlet?parentid=<%=itemId%>&commentid=<%=item.getId()%>&action=edit&src=AskItem.jsp" target = "_parent" style="display: none" ><br>
							...
							<input type="hidden" name="parentid" value="<%=itemId%>" /> 
  							...
  							<input type="hidden" name="action" value="edit" /> 
  							 <input type="hidden" name="src" value="AskItem.jsp" /> 
